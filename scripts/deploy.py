...
from scripts.front_end_helper import update_front_end
from brownie import CommunityPool, network, config, interface
from web3 import Web3

def deploy_and_set_contract():
    account = get_account()
    lender_contract = CommunityPool.deploy(
...
def use_contract():
    account = get_account()
    lender_contract = CommunityPool[-1]
    ...
    tx_deposit = lender_contract.poolDeposit(
        weth_token, 
        CONTRIBUTION, 
        {"from": account}
    )
